...
const draw2DMol = (id, molString, width, height) => {
...
const draw3DMol = (id, molString, width, height) => {
  const molecule = ChemDoodle.readMOL(molString);
...
const MolViewerSmall = ({ item, href, width = 200, height = 200 }) => {
  ...
  const handleData = (molString) => {
    if (mode == false) {
      draw2DMol(item.id, molString, width, height);
    } else {
      console.log('the mode is true!!!');
      draw3DMol(item.id, molString, width, height);
    }
  };
